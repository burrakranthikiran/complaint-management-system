...
error_reporting(E_ALL);
 ...
 <html>
   ...
   <div class="div">
     ...
     <?php
     $no=1;
       $db=mysql_query("SELECT * FROM `dummy` WHERE `user_name` = '$id_d'");
 ...
 <table>
 <?php
   $query1=mysql_query("SELECT * FROM `cmp_log` WHERE id='$comId'");
   ...
   if(($status)==46){
     $query = mysql_query("SELECT ref_no FROM `view_cmp` WHERE ref_no=$ref");
       if (mysql_num_rows($query) != 0){
         ...
         mysql_query("INSERT INTO `view_cmp` (ref_no, name, email, `phone no`, subject, complain, dummy, filename) VALUES ('$ref_e','$name_e','$email_e','$phone_no_e','$subject_e','$complain_e','$id_d','$fileName')") or die(mysql_error());
         ...
         <svg class='pointer' fill='#FFF' height='24' viewBox='0 0 24 24' width='24' xmlns='http://www.w3.org/2000/svg'>
           ...
           <p class='text-center'>Message successfully sent to the  Officer</p>
         </div> </div>";
       }
   }else{
          $message = "<div class='alert errr' id='msg'>
